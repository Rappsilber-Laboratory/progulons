...
pro <- read_xlsx("Supplementary_Table_3_Protein_mRNA_Profiles_36_Cell_Lines.xlsx", 
                 sheet = "Proteome_Profiles")
rna <- read_xlsx("Supplementary_Table_3_Protein_mRNA_Profiles_36_Cell_Lines.xlsx", 
                 sheet = "mRNA_Profiles")
...
write.csv(df$`Uniprot Gene Name`, "temp_Uniprot_IDs.csv")
id_conversion <- fread("ID_conversion.tab")
...
ggsave("Breast_cancer_histograms.pdf", pH, width = 14, height = 4.9, units = "cm")
...
p1
ggsave("Haas_Example_histograms.pdf", p1,
...
p2 <- ggplot(DT_summary, aes(x = med_RNA_RNA_PCC, y = med_pro_pro_PCC))+
        ...
        geom_text(data = DT_summary[ Progulon_ID %in% c("PRN01", "PRN04", "PRN07", "PRN08", "PRN10", "PRN11", "PRN21", "PRN24", "PRN26", "PRN30")],
...
p2 
ggsave("Haas_RNARNAvsProPro_scatter.pdf", p2,
...
p3
ggsave("Haas_RNAtoPro_Subset_boxplot.pdf", p3,
...
p4
ggsave("Haas_RNAtoPro_boxplot.pdf", p4,
...
fwrite( DT_summary , "Haas_Rna_Pro_PRN.csv" )
...
p5 <- ggplot(DT_summary, aes(x = med_mRNA_mad, y = med_prot_mad))+
        ...
        annotate(geom = "text", x = 0.02, y = 1.5, size = 2, hjust = 0, label = sigPCC )+      
...
p5 
ggsave("Haas_Scale_RNAvsPro.pdf", p5,
       width = 4.9, height = 4.9, units = "cm")
...
p6
ggsave("Haas_Scale_RNAtoPro.pdf", p6,
       width = 4.9, height = 4.9, units = "cm")
...
p7 <- ggplot(DT_summary, aes(x = med_RNA_RNA_PCC, y = med_mRNA_mad))+
      ...
      #           aes(label = prn_function), size = 2, colour = "blue", hjust = -0.1)+
      scale_x_continuous( limits = c(0,0.55), expand = c(0,0), breaks = seq(0,1,0.25))+
      scale_y_continuous( limits = c(0,0.2), expand = c(0,0), breaks = seq(0,1,0.1))+
      xlab("median mRNA coexpression [PCC]")+
      ylab("Scale of mRNA expression variation [median MAD]")+
      annotate(geom = "text", x = 0.3, y = 0.18, size = 2, hjust = 0, label = sigPCC )+      
      annotate(geom = "text", x = 0.3, y = 0.17, size = 2, hjust = 0, label = sigRHO )+      
      theme(plot.background = element_blank(), panel.background = element_rect(fill=NA, colour="black", size = 0.25),
            panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
            axis.title = element_text(size = 6), axis.text = element_text(size = 5, colour = "black"),
            legend.position = "none", axis.ticks = element_line(size = 0.25))
...
p7
ggsave("Haas_Scale_RNAtoRNA.pdf", p7,
       width = 4.9, height = 4.9, units = "cm")
...
p8 <- ggplot(DT_summary, aes(x = med_pro_pro_PCC, y = med_prot_mad))+
        ...
        ylab("Scale of protein expression variation [median MAD]")+
        annotate(geom = "text", x = 0.5, y = 1.7, size = 2, hjust = 0, label = sigPCC )+      
        annotate(geom = "text", x = 0.5, y = 1.6, size = 2, hjust = 0, label = sigRHO )+      
        theme(plot.background = element_blank(), panel.background = element_rect(fill=NA, colour="black", size = 0.25),
              panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
              axis.title = element_text(size = 6), axis.text = element_text(size = 5, colour = "black"),
              legend.position = "none", axis.ticks = element_line(size = 0.25))
...
p8
ggsave("Haas_Scale_ProToPro.pdf", p8,
       width = 4.9, height = 4.9, units = "cm")
...
ggsave("Haas_Scale_composite.pdf", g,
...
p11
ggsave("Haas_RNARNAvsmRNAPro.pdf", p11,
       width = 4.9, height = 4.9, units = "cm")
...
p12
ggsave("Haas_propro_vsmRNAPro.pdf", p12,
       width = 4.9, height = 4.9, units = "cm")
...
ggsave("Haas_combined_plot.pdf", combined_p,
