...
#### Load and prep the matched mRNA - protein input data for mouse tissues ####
...
df <- df[ df$Human_Uniprot %in% unique(prns$SimpleID) ,]

# Get the mRNA and protein ratios
rownames(df) <- df[, "Human_Uniprot"]
rna <- df[, grep("TPM_", colnames(df))]
pro <- df[, grep("SILAC_", colnames(df))]
...
prns <- prns[ SimpleID %in% df$Human_Uniprot ]


#### Get all possible pairwise mRNA - mRNA correlations in this dataset ####

RNA_RNA_cor <- cor(trna_mn, use = "pairwise.complete.obs", method = "pearson")   # All pairwise combinations
...
pro_pro_cor <- cor(tpro_mn, use = "pairwise.complete.obs", method = "pearson")  # All pairwise combinations
...
RNA_pro_cor <- cor(trna_mn, tpro_mn, use = "pairwise.complete.obs", method = "pearson")  # All pairwise combinations
...
pH2<- ggplot( DT[ Gene_1 == Gene_2 ] )+
        ...
        annotate("text", x = 0.6, y = 200, size = 2.5,
...
ggsave("Mouse_histograms.pdf", pH, width = 14, height = 4.9, units = "cm")
...
DT_summary <- DT_prn[, .(median_prn_PCC = median(PCC, na.rm = TRUE)), by = .(cor_type, Progulon_ID) ]
...
p1 <- ggplot( plot_dt1 , aes(x = PCC, fill = cor_type ))+
        ...
        geom_text( data = dt1_N_pairs, aes( label = N , x = -0.5, y = 65), size = 2)+
...
p1
ggsave("Mouse_Example_histograms.pdf", p1,
...
p2 <- ggplot(DT_summary, aes(x = med_RNA_RNA_PCC, y = med_pro_pro_PCC))+
        ...
        geom_text(data = DT_summary[ Progulon_ID %in% c("PRN01", "PRN04", "PRN07", "PRN08", "PRN10", "PRN11", "PRN21", "PRN24", "PRN26", "PRN29", "PRN30")],
...
p2 
ggsave("Mouse_RNARNAvsProPro_scatter.pdf", p2,
...
p3
ggsave("Mouse_RNAtoPro_Subset_boxplot.pdf", p3,
...
p4 <- ggplot( DT_prn[ cor_type == "RNA_pro_PCC" ], aes(x = prn_function, y = PCC ))+
        geom_boxplot(size = 0.25, outlier.fill = "white", outlier.shape = 21, outlier.stroke = 0.25, outlier.size = 0.5)+
        geom_hline( yintercept = DT_prn[ cor_type == "RNA_pro_PCC" , median(PCC), prn_function ][, mean(V1) ], linetype = "dashed", size = 0.25, colour = "grey50")+
        scale_y_continuous( limits = c(-0.9,1), expand = c(0,0), breaks = seq(-1,1,0.2))+
        ylab("mRNA to protein correlation [PCC]")+
        theme(plot.background = element_blank(), panel.background = element_rect(fill=NA, colour="black", size = 0.25),
              panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(size = 0.25, colour = "black"),
...
p4
ggsave("Mouse_RNAtoPro_boxplot.pdf", p4,
...
fwrite( DT_summary , "Mouse_Rna_Pro_PRN.csv" )
...
sigPCCest <- DT_summary[, cor.test(med_mRNA_mad, med_prot_mad, method = "pearson" )$estimate ] 
sigPCCpva <- DT_summary[, cor.test(med_mRNA_mad, med_prot_mad, method = "pearson" )$p.value  ] 
...
p5 <- ggplot(DT_summary, aes(x = med_mRNA_mad, y = med_prot_mad))+
        ...
        scale_x_continuous( limits = c(0.5,2), expand = c(0,0), breaks = seq(0,2,0.2))+
        scale_y_continuous( limits = c(0.3,1.8), expand = c(0,0), breaks = seq(0,2,0.2))+
        xlab("Scale of mRNA expression variation [median MAD]")+
        ylab("Scale of protein expression variation [median MAD]")+
        annotate(geom = "text", x = 0.6, y = 1.7, size = 2, hjust = 0, label = sigPCC )+      
        annotate(geom = "text", x = 0.6, y = 1.6, size = 2, hjust = 0, label = sigRHO )+      
        theme(plot.background = element_blank(), panel.background = element_rect(fill=NA, colour="black", size = 0.25),
              panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
              axis.title = element_text(size = 6), axis.text = element_text(size = 5, colour = "black"),
              legend.position = "none", axis.ticks = element_line(size = 0.25))
...
p5 
ggsave("Mouse_Scale_RNAvsPro.pdf", p5,
...
p6 <- ggplot(DT_summary, aes(x = med_RNA_pro_PCC, y = med_prot_mad))+
      geom_smooth( method = "lm", size = 0.25, colour = "orange")+
      geom_point( alpha = 0.5, size = 1 )+
      geom_text(data = DT_summary[ Progulon_ID %in% c("PRN01", "PRN08", "PRN12")],
                aes(label = prn_function), size = 2, colour = "blue", hjust = -0.1)+
      scale_x_continuous( limits = c(0,0.8), expand = c(0,0), breaks = seq(0,1,0.2))+
      scale_y_continuous( limits = c(0.3,1.8), expand = c(0,0), breaks = seq(0,2,0.2))+
      xlab("mRNA-to-protein correlation [median PCC]")+
      ylab("Scale of protein expression variation [median MAD]")+
      annotate(geom = "text", x = 0.1, y = 1.6, size = 2, hjust = 0, label = sigPCC )+      
...
p6
ggsave("Mouse_Scale_RNAtoPro.pdf", p6,
       width = 4.5, height = 4.5, units = "cm")
...
p7 <- ggplot(DT_summary, aes(x = med_RNA_RNA_PCC, y = med_mRNA_mad))+
      ...
      #           aes(label = prn_function), size = 2, colour = "blue", hjust = -0.1)+
      scale_x_continuous( limits = c(0,0.8), expand = c(0,0), breaks = seq(0,1,0.2))+
      scale_y_continuous( limits = c(0.5,1.8), expand = c(0,0), breaks = seq(0,2,0.2))+
      xlab("median mRNA coexpression [PCC]")+
      ylab("Scale of mRNA expression variation [median MAD]")+
      annotate(geom = "text", x = 0.6, y = 1.6, size = 2, hjust = 0, label = sigPCC )+      
      annotate(geom = "text", x = 0.6, y = 1.5, size = 2, hjust = 0, label = sigRHO )+      
      theme(plot.background = element_blank(), panel.background = element_rect(fill=NA, colour="black", size = 0.25),
            panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
            axis.title = element_text(size = 6), axis.text = element_text(size = 5, colour = "black"),
            legend.position = "none", axis.ticks = element_line(size = 0.25))
...
p7
ggsave("Mouse_Scale_RNAtoRNA.pdf", p7,
       width = 4.5, height = 4.5, units = "cm")
...
p8 <- ggplot(DT_summary, aes(x = med_pro_pro_PCC, y = med_prot_mad))+
        ...
        geom_text(data = DT_summary[ Progulon_ID %in% c("PRN01", "PRN04", "PRN07", "PRN08", "PRN11", "PRN26", "PRN29")],
                  aes(label = prn_function), size = 2, colour = "blue", hjust = -0.1)+
        scale_x_continuous( limits = c(-0.05,1), expand = c(0,0), breaks = seq(0,1,0.2))+
        scale_y_continuous( limits = c(0.45,1.65), expand = c(0,0), breaks = seq(0,2,0.2))+
        xlab("median protein coexpression [PCC]")+
        ylab("Scale of protein expression variation [median MAD]")+
        annotate(geom = "text", x = 0.65, y = 1.5, size = 2, hjust = 0, label = sigPCC )+      
        annotate(geom = "text", x = 0.65, y = 1.4, size = 2, hjust = 0, label = sigRHO )+      
        theme(plot.background = element_blank(), panel.background = element_rect(fill=NA, colour="black", size = 0.25),
              panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
              axis.title = element_text(size = 6), axis.text = element_text(size = 5, colour = "black"),
              legend.position = "none", axis.ticks = element_line(size = 0.25))
...
p8
ggsave("Mouse_Scale_ProToPro.pdf", p8,
       width = 4.5, height = 4.5, units = "cm")
...
ggsave("Mouse_Scale_composite.pdf", g,
...
p10 <- ggplot(DT_summary, aes(x = med_RNA_pro_PCC, y = -log10(p_RNAtoPRO)))+
  geom_point( alpha = 0.5, size = 1 )+
  geom_vline( xintercept = DT[ Gene_1 == Gene_2 , median(RNA_pro_PCC) ], size = 0.25, linetype = "dashed")+
  geom_text(data = DT_summary[ order(med_RNA_pro_PCC) ][ c(1:15, 33:41) ],
...
p11 <- ggplot(DT_summary, aes(x = med_RNA_RNA_PCC, y = med_RNA_pro_PCC))+
        ...
        scale_y_continuous( limits = c(0,0.81), expand = c(0,0), breaks = seq(0,1,0.2))+
        ylab("mRNA to protein correlation [PCC]")+
...
p11
ggsave("Mouse_RNARNAvsmRNAPro.pdf", p11,
       width = 4.9, height = 4.9, units = "cm")
...
p12 <- ggplot(DT_summary, aes(x = med_pro_pro_PCC, y = med_RNA_pro_PCC))+
        ...
        geom_text(data = DT_summary[ order(med_RNA_pro_PCC) ][ c(1:5, 37:41) ],
                  aes(label = prn_function), size = 2, colour = "blue", hjust = 1, angle = 90)+
        scale_x_continuous( limits = c(-0.05,0.91), expand = c(0,0), breaks = seq(0,1,0.2))+
        scale_y_continuous( limits = c(0,0.81), expand = c(0,0), breaks = seq(0,1,0.2))+
        ylab("mRNA to protein correlation [PCC]")+
...
p12
ggsave("Mouse_propro_vsmRNAPro.pdf", p12,
       width = 4.9, height = 4.9, units = "cm")
...
ggsave("Mouse_combined_plot.pdf", combined_p,
