...
# Load ENSEMBL genome coordinates of human genes. The file "Human_gene_positions_26Mar18.csv" was downloaded from www.ensembl.org using BioMart.
# In BioMart, choose database “Ensembl Genes 91”, dataset “Human genes (GRCh38.p10)”, filter for Chromosomes 1-22, X, Y and limit to genes with
# Uniprot/Swissprot accessions. Select the attributes GeneID, Gene start (bp), Gene end (bp), Chromosome Name, Strand, and (external) UniprotKB/
# Swissprot ID. Export unique results.
Genes <- fread("Human_gene_positions_26Mar18.csv")
...
PRN21_ATPs_genes <- DT[ Progulon_ID == "PRN21" , .( unique( c( Gene_1, Gene_2 ))) ]
PRN29_Nuc_genes <- DT[ Progulon_ID == "PRN29" ,  .( unique( c( Gene_1, Gene_2 ))) ]

fwrite(PRN21_ATPs_genes, "PRN21_ATPs_genes.csv")
fwrite(PRN29_Nuc_genes, "PRN29_Nuc_genes.csv")
...
p <- ggplot(progulons_chr_enrichment, aes(x = pc_gp_sp_sChr, y = -log10(pvalue)))+
      ...
      geom_point(data = progulons_chr_enrichment[ Progulon_ID %in% c("PRN21", "PRN29")],
                 size = 1, colour = "green")+
      geom_text(data = progulons_chr_enrichment[ Progulon_ID %in% c("PRN21", "PRN29")],
                aes(label = Progulon_ID), size = 2, hjust = -0.3)+
...
ggsave("Position_volcano.pdf", p,
       width = 3, height = 4.9, units = "cm")
