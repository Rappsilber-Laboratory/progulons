...
PRN_replisome <- fread("RF_score_Replisome_all.csv")
...
GO <- fread("QuickGO_DNA_replication_GO0006260.tsv")
...
ggsave("RF_plot.pdf", p3, width=4.7, height=5.5, units= "cm", dpi = 600)
...
pSNE
ggsave("Replisome_tSNE.pdf", pSNE, width=9.45, height=6, units= "cm")

# Display an interactive version of this plot, showing gene names as tooltip
# ggplotly(pSNE, tooltip = "Gene_names")
...
pScreen
ggsave("Main_screen_results.pdf", pScreen, width=9, height=5, units= "cm")
...
PRN_without_NCC <- fread("RF_score_Replisome_without_NCC.csv")
...
PRN_without_15_ran_1 <- fread("RF_score_Replisome_without15random_1.csv")
...
# Load a progulonFinder result that was run on ProteomeHD minus 15 ratios chose at random
PRN_without_15_ran_2 <- fread("RF_score_Replisome_without15random_2.csv")
...
# Load a progulonFinder result that was run on ProteomeHD minus 15 ratios chose at random
PRN_without_15_ran_3 <- fread("RF_score_Replisome_without15random_3.csv")
...
ggsave("Without_NCC_or_random.png", p4, width=18.3, height=5, units= "cm", dpi = 600)
...
PRN_NCC_only <- fread("RF_score_Replisome_NCConly.csv")
...
p5 <- ggplot(PRN_replisome, aes(y = Mean_RF_score, x = RF_score_NCC_only, colour = Screening_type, label = Gene_names))+
        ...
        geom_point(data = PRN_replisome[ Used_for_positive_training == "Used"], colour = "black", fill=NA, size = 0.5, shape = 1, stroke=0.2)+
        geom_point(data = PRN_replisome[ !is.na(Screening_type) ], size = 0.2, show.legend = FALSE)+ 
        geom_text( data = PRN_replisome[ Screening_type == "candidate_from_NCC_exps" ], aes( colour = Screening_type), size = 1.7 , show.legend = FALSE)+
...
pScreen2 <- ggplot( siRNA_results[ Type_of_candidate == "candidate_from_NCC_exps" ],
                   aes(x = Candidate, y = SD_score, fill = Process ))+
            geom_bar( stat = "identity" )+
            geom_hline( yintercept = 13/2, linetype = "dashed", size = 0.25, colour = "grey50")+
            geom_hline( yintercept = 13/3, linetype = "dashed", size = 0.25, colour = "grey50")+
...
ggsave("NCC_screen_results.pdf", pNCC_only, width=18.3, height=12, units= "cm")
...
PRN_ran15_1 <- fread("RF_score_Replisome_only15random_1.csv")
PRN_ran15_2 <- fread("RF_score_Replisome_only15random_2.csv")
PRN_ran15_3 <- fread("RF_score_Replisome_only15random_3.csv")
...
p6 <- ggplot(bxpl_data, aes(x = variable, y = value, fill = Boxplot_annotation))+
        geom_boxplot(notch = TRUE, outlier.size = 0.2, size = 0.25)+
...
p6
ggsave("NCC_only_boxplot.pdf", p6, width = 18.3, height = 5, units= "cm")
...
fwrite(PRN_replisome, "Replisome_predictions.csv")
